...
export const computeGlobalParams = (
  ...
  const previousType =
    items.length > 0 ? (items[0].type as "filter" | "dimension") : "dimension";
...
export const processAnalyticsData = ({
  ...
  if (rows) {
    values = fromPairs(
      rows.map((r) => [r.slice(0, r.length - 1).join(""), r[r.length - 1]])
    );
  }
      ...
      for (const item1 of second) {
        for (const item2 of third) {
          currentData.push({
            ...
            [`${label2}-name`]: metaData.items[item2]?.name,
            value: values[`${item0}${item1}${item2}`] || options.valueIfEmpty,
          });
        }
      }
    }
...
export const findParameters = (visualization: any) => {
  const colors =
    colorSets[visualization.colorSet]?.colors || colorSets["DEFAULT"].colors;
