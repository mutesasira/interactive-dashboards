...
export const useInitials = (storage: "data-store" | "es") => {
  ...
  const ouQuery = {
    me: {
      ...
      params: {
        fields:
          "dataViewOrganisationUnits[id,name,leaf,level],authorities,userRoles[name]",
      },
    },
    levels: {
  ...
  return useQuery<string, Error>(
    ...
    async ({ signal }) => {
      ...
      const isAdmin =
        ...
        (userRoles !== undefined &&
          userRoles.map(({ name }: any) => name).indexOf("Superuser") !== -1);
  ...
  return useQuery<boolean, Error>(
    ...
    async ({ signal }) => {
      if (action === "update" || action === "view") {
        const dataSource = await getOneRecord<IDataSource>(storage, id, {
          namespace: "i-data-sources",
          otherQueries: [],
          signal,
          engine,
...
export const useCategoryList = (
  ...
  const engine = useDataEngine();
  return useQuery<{ dashboards: IDashboard[]; categories: ICategory[] }, Error>(
    ["i-dashboards-categories"],
    async ({ signal }) => {
      const dashboards = await getIndex<IDashboard>(storage, {
        namespace: "i-dashboards",
        systemId,
...
export const useDashboard = (
  ...
  const params = {
    fields:
      "categories[id,name,shortName,categoryOptions[id,name,startDate,endDate]],categoryOptionCombos[id,categoryOptions]",
  };
  return useQuery<IDashboard, Error>(
    ["i-dashboards", id],
    async ({ signal }) => {
...
export const useCategory = (storage: "data-store" | "es", id: string) => {
  ...
  return useQuery<boolean, Error>(["i-categories", id], async ({ signal }) => {
    try {
...
export const useDHIS2CategoryCombo = (
  ...
  return useQuery<CategoryCombo, Error>(["category-combo", id], async () => {
    ...
    categoryCombo.categories.forEach((c) => {
      const valid = c.categoryOptions.filter((a) => a.endDate === undefined);
      attributionApi.add({
        [c.id]: valid.map((e) => e.id).join(","),
      });
    });
    ...
    ([key, { resource, type, dimension, prefix }]) => {
      ...
      if (globalValue) {
        return {
          ...
          type,
          dimension,
          value: globalValue.map((a: any) => `${prefix || ""}${a}`).join(";"),
        };
      }
      return {
        resource,
...
const queryData = async (
  ...
  if (vq?.dataSource?.type === "DHIS2" && vq.type === "ANALYTICS") {
    ...
    if (
      ...
      Object.entries(joinData.metaData.dimensions).forEach(([key, values]) => {
        if (dimensions[key]) {
          dimensions = {
            ...dimensions,
            [key]: uniq([...dimensions[key], ...(values as string[])]),
          };
        } else {
          dimensions = {
            ...dimensions,
...
const queryDHIS2 = async (
  ...
  if (vq) {
    ...
    if (vq.dataSource && vq.dataSource.type === "ELASTICSEARCH" && vq.query) {
...
const queryIndicator = async (
  ...
  const {
    ...
    dimensions,
    metadata,
  } = await queryData(engine, indicator.numerator, globalFilters, otherFilters);
  ...
  if (numerator && denominator) {
    const data = numerator.map((currentValue: { [key: string]: string }) => {
      ...
      const denominatorSearch = denominator.find(
        (row: { [key: string]: string }) => {
          ...
          return columns === Object.values(someOthers).sort().join("");
        }
      );
      ...
      } else if (denominator.length === 1 && numerator.length === 1) {
        const { value: v2, total: t2 } = denominator[0];
        const numeratorValue = v1 || t1;
        const denominatorValue = v2 || t2;
        return computeIndicator(
          indicator,
...
export const useVisualizationMetadata = (
  ...
  return useQuery<IVisualization2, Error>(
    ["visualizations-metadata", visualization.id, ...visualization.indicators],
    async ({ signal }) => {
      ...
      const processedIndicators: Array<IIndicator2> = indicators.map((i) => {
        let numerator1 = queries.find((q) => q.id === i.numerator);
        let denominator1 = queries.find((q) => q.id === i.denominator);
        let numerator: IData2 | undefined = undefined;
        let denominator: IData2 | undefined = undefined;

        if (numerator1) {
          let joiner = queries.find((q) => q.id === numerator1?.joinTo);
        ...
        if (denominator1) {
          let joiner = queries.find((q) => q.id === denominator1?.joinTo);
          let joinTo: IData2 | undefined = undefined;
...
const queryTree = async (engine: any, dashboard: IDashboard) => {
  ...
  if (dashboard.filters) {
    for (const filter of dashboard.filters) {
      ...
      const children = dashboard.filters.filter((f) => f.parent === filter.id);
      if (parents.length > 0) {
            ...
            value,
            {
              resource:
                String(filter.resource).indexOf("${q}") !== -1
                  ? String(filter.resource).replace("${q}", value ?? "")
                  : filter.resource,
            },
          ])
        );
        ...
        for (const { value, key, id: parentId } of parents) {
          ...
          } else if (currentData && currentData.dataElementGroupSets) {
            const current = currentData.dataElementGroupSets.flatMap(
              ({
                code: degsCode,
                id: degsId,
                name: degsName,
                ...
                if (String(degsCode).split(".").length === 2) {
                  return dataElementGroups.map(
                    ({ code: degCode, id: degId, name: degName }: any) => {
                      const node: DataNode = {
                        pId: String(parentId),
                        nodeSource: {
                          search: filter.resourceKey,
        ...
        if (data && data.options) {
          const current = data.options.map(({ code, id, name }: any) => {
            const node: DataNode = {
              pId: dashboard.id,
  ...
  >(
    ...
    async () => {
      if (viz.dataSource && viz.properties?.["visualization"]) {
        ...
        for (const [ke, values] of Object.entries(metaData.dimensions)) {
          vals = {
            ...vals,
