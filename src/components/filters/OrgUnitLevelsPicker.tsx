...
const OrgUnitLevelsPicker = () => {
    ...
    const selectedOptions = organisations.filter(
        (pt) => selectedLevels.indexOf(String(pt.value)) !== -1
    );

    const onChangeLevels = (levels: Option[] | null) => {
        storeApi.changeLevels(levels?.map((ex) => String(ex.value)) || []);
    };

    const getButtonLabel = () => {
        if (selectedOptions.length === 0) {
            return "Select Organization Unit Levels";
        } else if (selectedOptions.length === 1) {
            return selectedOptions[0].label;
        } else if (selectedOptions.length <= 2) {
            return selectedOptions.map(o => o.label).join(", ");
        } else {
            return `${selectedOptions.length} levels selected`;
        }
    };
                ...
                >
                    <Box px="3" py="2" textAlign="right" borderBottom="1px solid" borderColor="gray.200" bg="gray.50">
                        <Text fontSize="md" fontWeight="semibold" float="left" mt="1">
                            Select Organization Unit Levels
                        </Text>
                        <Button
                            size="xs"
                            colorScheme="gray"
                    ...
                    <Box p="3" overflow="auto" maxH="240px">
                        <Select<Option, true, GroupBase<Option>>
                            ...
                            isClearable
                            isMulti
                            placeholder="Select organization unit levels..."
                            menuIsOpen={false}
                            components={{
...
export default OrgUnitLevelsPicker;