...
export const processTable = (
  ...
  if (data) {
    ...
    const finalData = allKeys.sort().map((key) => {
      ...
      Object.entries(groupedByColumn).forEach(([columnKey, columnData]) => {
        const calculated = calculation[aggregation](columnData, thresholds, {
          aggregationColumn,
...
export const processGraphs = (
  ...
  const percentages: boolean = options.dataProperties?.["percentages"] || false;
  ...
  if (data && data.length > 0 && options.category !== undefined) {
    if (options.summarize) {
      ...
      if (options.series) {
        ...
        chartData = Object.entries(grouped).flatMap(([key, values]) => {
          ...
          if (specific.length > 0) {
            currentValues = values.filter(
              (d: any) => specific.indexOf(d[options.series || ""]) !== -1
            );
            ...
            if (specific.indexOf(key) !== -1) {
              return {
                ...
                x: Object.keys(groupedByCategory)
                  .sort((a, b) => {
                    const pa = Number(
                      options.dataProperties[`data.${a}.position`] ?? 0
                    );
                    const pb = Number(
                      options.dataProperties[`data.${b}.position`] ?? 0
                    );
                    return pa - pb;
                  })
                  .map((k) =>
                ...
                y: Object.keys(groupedByCategory)
                  ...
                  .map((k) => {
                    ...
                    if (percentages && overall) {
                      return (
                        (current.length * 100) / groupedByCategory[k].length
                      );
                    }
                    if (percentages) {
                      return (
          ...
          return {
            ...
            y: Object.keys(groupedByCategory)
              ...
              .map((k) => {
                ...
                if (percentages && overall) {
                  return (current.length * 100) / groupedByCategory[k].length;
                }
                if (percentages) {
                  return (
                    (current.length * 100) /
        ...
        allSeries = Object.keys(
          groupBy(
            data.filter((d: any) => {
              if (specific.length > 0) {
                return specific.indexOf(d[options.series || ""]) !== -1;
              }
              return true;
            }),
            options.series
      ...
      const columns = orderBy(
        x.map((c: any) => {
          const name = allMetadata[c] || options.metadata?.[c]?.name || c;
          return {
            id: c,
            name,
            ...(availableProperties.data[name] ?? { position: 0 }),
          };
        }),
        "position",
        "asc"
